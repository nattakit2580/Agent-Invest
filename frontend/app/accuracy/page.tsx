...
export default function AccuracyPage() {
  ...
  if (!stats) return <div className="text-red-600 p-8">ไม่สามารถโหลดข้อมูลได้</div>;
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-slate-500 mt-1">ติดตามประสิทธิภาพการคาดการณ์ของ AI</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          icon={TrendingUp}
          color={stats.direction_accuracy >= 0.7 ? "text-emerald-600" : stats.direction_accuracy >= 0.5 ? "text-yellow-500" : "text-red-600"}
        />
        <StatCard
          title="คะแนนเฉลี่ย"
      ...
      {stats.compared === 0 && (
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-8 text-center text-slate-500">
          ยังไม่มีการเทียบผล — ไปที่หน้า <a href="/predictions" className="text-blue-600 hover:underline">ประวัติ</a> แล้วกด "เทียบกับราคาปัจจุบัน"
        </div>
      )}

      {tfData.length > 0 && (
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
          <h2 className="text-lg font-semibold text-slate-900 mb-5">ความแม่นยำตาม Timeframe</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
            {tfData.map((d) => (
              <div key={d.name} className="bg-slate-50 rounded-lg p-3 text-center border border-slate-200">
                <div className="text-slate-500 text-xs">{d.name}</div>
                <div className={`text-xl font-bold ${accuracyColor(d.accuracy) === "#10b981" ? "text-emerald-600" : accuracyColor(d.accuracy) === "#f59e0b" ? "text-yellow-500" : "text-red-600"}`}>
                  {d.accuracy}%
                </div>
                <div className="text-slate-400 text-xs">{d.total} รายการ</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {symData.length > 0 && (
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
          <h2 className="text-lg font-semibold text-slate-900 mb-5">ความแม่นยำตาม Symbol (Top 10)</h2>
