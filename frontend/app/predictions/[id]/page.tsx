...
function AgentCard({ name, output }: { name: string; output: Record<string, unknown> }) {
  ...
  return (
    <div className={`bg-slate-800 border rounded-xl p-5 ${
      dir === "bullish" ? "border-emerald-700" : dir === "bearish" ? "border-red-700" : "border-slate-600"
    }`}>
      ...
      <div className="w-full bg-slate-700 rounded-full h-1.5 mb-3">
        <div className={`h-1.5 rounded-full ${dir === "bullish" ? "bg-emerald-500" : dir === "bearish" ? "bg-red-500" : "bg-slate-500"}`}
          style={{ width: `${conf * 100}%` }} />
      </div>
      <p className="text-slate-300 text-xs mb-3">{output.summary as string}</p>
      <ul className="space-y-1">
        {((output.key_points as string[]) || []).map((pt, i) => (
          <li key={i} className="text-slate-500 text-xs flex gap-1"><span className="text-sky-500">•</span>{pt}</li>
        ))}
      </ul>
      {Object.entries(output).filter(([k]) => !["direction","confidence","summary","key_points"].includes(k)).length > 0 && (
        <div className="mt-3 pt-3 border-t border-slate-700 flex flex-wrap gap-2">
          {Object.entries(output)
            ...
            .map(([k, v]) => (
              <span key={k} className="text-xs bg-slate-700 text-slate-400 px-2 py-0.5 rounded">
...
export default function PredictionDetailPage() {
  ...
  if (!prediction) return <div className="text-red-400 p-8">ไม่พบข้อมูล</div>;
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className={`rounded-xl p-6 border-2 ${
        p.direction === "bullish" ? "bg-emerald-900/20 border-emerald-600" :
        p.direction === "bearish" ? "bg-red-900/20 border-red-600" :
        "bg-slate-800 border-slate-600"
      }`}>
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="text-3xl font-bold text-white">{p.symbol}</h1>
              <DirectionBadge direction={p.direction} />
              <span className="text-slate-400 text-sm">{p.timeframe}</span>
            </div>
            <p className="text-slate-300 mt-3 leading-relaxed">{p.reasoning}</p>
            <p className="text-slate-500 text-xs mt-2">{format(new Date(p.created_at), "dd MMM yyyy HH:mm")}</p>
          </div>
          <div className="text-right ml-6 flex-shrink-0 space-y-2">
            <div>
              <div className="text-slate-400 text-xs">Confidence</div>
              <div className="text-4xl font-bold text-white">{(p.confidence * 100).toFixed(0)}%</div>
            </div>
            <div>
              <div className="text-slate-400 text-xs">Entry Price</div>
              <div className="text-white font-semibold text-lg">${p.current_price.toFixed(2)}</div>
            </div>
            {p.target_price && (
              <div>
                <div className="text-slate-400 text-xs">Target</div>
                <div className="text-sky-400 font-semibold">${p.target_price.toFixed(2)}</div>
              </div>
            )}
          </div>
        </div>
      ...
      {p.status === "compared" && (
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
          <h2 className="text-lg font-semibold text-white flex items-center gap-2 mb-4">
            <CheckCircle className="w-5 h-5 text-emerald-400" /> ผลการเทียบ
          </h2>
          <div className="grid grid-cols-3 gap-4">
            <div className="text-center">
              <div className="text-slate-400 text-xs mb-1">ราคาจริง</div>
              <div className="text-white text-xl font-bold">${p.actual_price?.toFixed(2)}</div>
            </div>
            <div className="text-center">
              <div className="text-slate-400 text-xs mb-1">ทิศทางจริง</div>
              <DirectionBadge direction={p.actual_direction ?? "neutral"} />
            </div>
            <div className="text-center">
              <div className="text-slate-400 text-xs mb-1">คะแนนความแม่นยำ</div>
              <div className={`text-2xl font-bold ${
                (p.accuracy_score ?? 0) >= 0.7 ? "text-emerald-400" :
                (p.accuracy_score ?? 0) >= 0.4 ? "text-yellow-400" : "text-red-400"
              }`}>
      ...
      {p.status === "pending" && (
        <button
          ...
          disabled={comparing}
          className="w-full bg-sky-600/20 hover:bg-sky-600/30 border border-sky-600 text-sky-400 rounded-xl py-3 flex items-center justify-center gap-2 transition-colors"
        >
      ...
      {p.agent_outputs && (
        <div>
          <h2 className="text-lg font-semibold text-white mb-3">ผลวิเคราะห์แยก Agent</h2>
