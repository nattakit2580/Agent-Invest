...
def compare_prediction(prediction_id: str, req: CompareRequest, db: Session = Depends(get_db)):
    ...
    actual_direction = calc_direction_from_prices(p.current_price, req.actual_price)
    score = calc_accuracy_score(
        p.direction, actual_direction, p.target_price, req.actual_price, p.current_price, p.confidence
    )

    p.actual_price = req.actual_price
